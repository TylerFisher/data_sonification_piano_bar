...
class bomb2midi(object):
    ...
    def remove_weeks(self, csv_obj):
        return [r for r in csv_obj if r['week'] not in ['']]
    ...
    def csv_to_miditime(self):
        raw_data = list(self.read_csv('data/prince.csv'))
        filtered_data = self.remove_weeks(raw_data)

        self.mymidi = MIDITime(self.tempo, 'prince_test.mid', self.seconds_per_year, self.base_octave, self.octave_range, self.epoch)

        self.minimum = self.mymidi.get_data_range(filtered_data, 'songs')[0]
        self.maximum = self.mymidi.get_data_range(filtered_data, 'songs')[1]

        timed_data = []

        for r in filtered_data:
            python_date = datetime.strptime(r["week"], "%Y-%m-%d")
        ...
        note_list = self.make_notes(timed_data, 'songs')
